...
def _paper_payload(session: SessionDep, paper_id: int) -> Dict[str, Any]:
    ...
    if author_ids:
        ...
        for a in authors:
            # 某些库可能没有 orcid/affiliation 字段，容错处理
...
def list_papers(session: SessionDep, q: Optional[str] = None, tag_id: Optional[int] = None, dedup: bool = True):
    stmt = select(Paper)
    if q:
...
async def upload_paper(
    ...
    # 5) 作者写库（解析到的 authors 优先）
    authors_meta = meta.get("authors") or []
    # 如果前端也传了 author_ids，两者合并
    ...
    for am in authors_meta:
        ...
        if not a:
            try:
                a = Author(name=name, affiliation=aff)  # 如果模型没有 affiliation 字段，ORM 会报错——用户模型里通常是有的
            except TypeError:
                a = Author(name=name)  # 容错：无该字段时退化
            session.add(a)
            session.commit()
            session.refresh(a)
        else:
...
async def upload_batch(session: SessionDep, files: list[UploadFile] = File(...)):
    ...
    for f in files:
        # 复用单文件上传逻辑
        tmp = await upload_paper(session=session, file=f, title=None, abstract=None, year=None, doi=None, venue=None, tag_ids=None, author_ids=None)  # type: ignore
        created.append(tmp)
    return created
